...
get_options(int argc, char *argv[])
{
	...
	for (s = argv[0]+1; *s; s++)
		...
		++argv;
		deleted_verts = argv;
		for (num_deleted_verts = 0;
		     ((*argv) && (sscanf((*argv), "%d\n", &dummy) == 1));
		     num_deleted_verts++)
		    ++argv;
		--argv;
		argc -= num_deleted_verts;
		break;
	    default:
		usage (progname);
		exit (-1);
...
delete_verts()
{
    ...
    for (i=0; i<nverts; i++)
	vlist[i]->deleted = FALSE;
    
    
    for (i=0; i<num_deleted_verts; i++)
    {
	if ((sscanf(deleted_verts[i], "%d", &id) != 1) ||
	    (id < 0) || (id >= nverts))
	{
	    fprintf(stderr, "Invalid vert id\n");
	    exit(-1);
	}
	vlist[id]->deleted = TRUE;
    }
	...
	for (j=0; j<face->nverts; j++)
	    if (vlist[face->verts[j]]->deleted == TRUE)
		face->deleted = TRUE;
    }
